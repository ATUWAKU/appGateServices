package com.appgate.prueba.utils.constants;

public class General {

    public final static String BASE_URL_API = "http://api.geonames.org/";
    public final static String TIME_ZONE_API = "timezoneJSON?formatted=true&lat=%s&lng=%s&username=%s&style=full";
    public final static String FRONTEND = "FRONTEND";
    public final static String USER_API = "atuwaku";
    public final static String RESPONSE = "Respuesta del llamado al servicio";


}
